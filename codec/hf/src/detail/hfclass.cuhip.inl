...
// Helper function to convert Huffman code to binary string
std::string huffman_code_to_binary(uint32_t code, uint8_t bit_count) {
  std::string binary_str = "";
  for (int bit = bit_count - 1; bit >= 0; --bit) {
    binary_str += ((code >> bit) & 1) ? '1' : '0';
  }
  return binary_str;
}
...
PHF_TPL void PHF_CLASS::print_huffman_codes() {
  // ------------------------------------
  // 4-BYTE CODE PATH
  // ------------------------------------
  size_t bk_size = pimpl->bklen * sizeof(u4);
  u4* h_bk = new u4[pimpl->bklen];
  if (!h_bk) {
    fprintf(stderr, "Failed to allocate host memory for 4-byte Huffman Book.\n");
    exit(EXIT_FAILURE);
  }
  ...
  CHECK_CUDA(cudaMemcpy(
      h_bk,
      pimpl->buf->bk4->dptr(),  // 4-byte buffer
      bk_size,
      cudaMemcpyDeviceToHost
      ));

  printf("Huffman Codes (4-byte):\n");
  for (int i = 0; i < pimpl->bklen; ++i) {
    u4 entry = h_bk[i];

    // Skip if top bit (bit 31) => invalid
    if ((entry >> 31) == 1) {
      continue;
    }
    // Bits [30..27] => code length (4 bits)
    int code_length = (entry >> 27) & 0xF;
    // Lower bits => code
    u4 code_mask = (1 << code_length) - 1;
    u4 code = entry & code_mask;

    std::bitset<32> bits(code);
    std::string binary_code = bits.to_string().substr(32 - code_length);
    printf("Symbol %d: Code Len = %d, Code = %s\n", i, code_length, binary_code.c_str());
  }
